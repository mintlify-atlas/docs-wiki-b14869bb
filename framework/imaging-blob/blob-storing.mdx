...
The provider modules ([FileSystem](/framework/imaging-blob/blob-storing-filesystem), [Azure](/framework/imaging-blob/blob-storing-azure), [AWS S3](/framework/imaging-blob/blob-storing-aws), [MinIO](/framework/imaging-blob/blob-storing-minio), [Aliyun OSS](/framework/imaging-blob/blob-storing-aliyun), and the application-level [Database](/framework/imaging-blob/blob-storing-database) module) plug in by registering an `IBlobProvider` and exposing a `UseXxx(...)` extension method on `BlobContainerConfiguration`.
...
2. **Normalizes both names.** `BlobNormalizeNamingService` runs every `IBlobNamingNormalizer` registered on the configuration in order, against the **container name** and the **blob name** separately. Provider modules register their own normalizers — see [Azure](/framework/imaging-blob/blob-storing-azure), [AWS](/framework/imaging-blob/blob-storing-aws), [MinIO](/framework/imaging-blob/blob-storing-minio).
...
- [Volo.Abp.BlobStoring.FileSystem](/framework/imaging-blob/blob-storing-filesystem)
- [Volo.Abp.BlobStoring.Azure](/framework/imaging-blob/blob-storing-azure)
- [Volo.Abp.BlobStoring.Aws](/framework/imaging-blob/blob-storing-aws)
- [Volo.Abp.BlobStoring.Minio](/framework/imaging-blob/blob-storing-minio)
- [Volo.Abp.BlobStoring.Aliyun](/framework/imaging-blob/blob-storing-aliyun)
- [Volo.Abp.BlobStoring.Database (module)](/framework/imaging-blob/blob-storing-database)
- [Volo.Abp.Imaging](/framework/imaging-blob/imaging) for the image-processing pipeline that often pairs with BLOB containers.
