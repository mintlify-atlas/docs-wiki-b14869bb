...
1. Sets `ProviderType = typeof(FileSystemBlobProvider)`. The [provider selector](blob-storing#provider-selection) uses this to match.
...
Compare with [Azure](blob-storing-azure#multi-tenant-and-container-name-strategy) and [AWS](blob-storing-aws#multi-tenant-and-container-name-strategy), which support a separate `ContainerName` property because the storage backend has a separate namespace concept.
...
- **Multi-node deployments need a shared filesystem.** The Polly retry handles transient file-locks, not network-partition cases. For multi-instance hosts pick [Azure](blob-storing-azure), [AWS](blob-storing-aws), [MinIO](blob-storing-minio), or [Aliyun OSS](blob-storing-aliyun).

## See also

- [Volo.Abp.BlobStoring](blob-storing) — the abstractions and `BlobContainer` pipeline.
- [Volo.Abp.BlobStoring.Database (module)](blob-storing-database) — for persistence inside your existing RDBMS.
- [Volo.Abp.BlobStoring.Azure](blob-storing-azure), [.Aws](blob-storing-aws), [.Minio](blob-storing-minio), [.Aliyun](blob-storing-aliyun) — cloud alternatives.
