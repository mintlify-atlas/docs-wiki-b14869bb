...
The caching dependency is for `IDistributedCache<AliyunTemporaryCredentialsCacheItem>` — the STS credentials are cached the same way as in [AWS](blob-storing-aws#sts-caching-with-encryption-at-rest).
...
The notable difference from [AWS](blob-storing-aws#awsblobnamingnormalizer) is **dots are stripped**, not preserved. OSS bucket names must match `[a-z0-9-]{3,63}` and start/end with an alphanumeric. So `my.app-prod` becomes `myapp-prod`.
...
- **`Endpoint` includes the scheme.** Unlike [MinIO](blob-storing-minio), which expects `host:port` without a scheme, the OSS SDK expects a full URL (`https://oss-cn-hangzhou.aliyuncs.com`). The string is passed verbatim into `new OssClient(endpoint, …)`.
...
- [Volo.Abp.BlobStoring](blob-storing) — the consumer pipeline.
- [Volo.Abp.BlobStoring.Aws](blob-storing-aws) — same STS-cache pattern, async SDK.
- [Volo.Abp.BlobStoring.Minio](blob-storing-minio) — S3-compatible alternative when you don't need Aliyun OSS specifically.
