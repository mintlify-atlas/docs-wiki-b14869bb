...
The standard `DefaultContainer` sentinel from the [core](blob-storing#the-two-iblobcontainer-shapes) module still applies: injecting raw `IBlobContainer` resolves the configuration named `"default"`. With the database provider, that name is just a string in the database.
...
- **Every save loads the whole blob into managed memory.** This provider is great for small attachments (kilobytes to single-digit megabytes) and a poor choice for video or large datasets. Use [Azure](blob-storing-azure), [AWS](blob-storing-aws), [MinIO](blob-storing-minio), or [Aliyun](blob-storing-aliyun) for large objects.
...
- [Volo.Abp.BlobStoring](blob-storing) — the consumer API every provider plugs into.
- [Volo.Abp.BlobStoring.FileSystem](blob-storing-filesystem) — equivalent for single-node disk storage.
