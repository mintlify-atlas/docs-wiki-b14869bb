...
- [Unit of Work](/framework/data/uow) — uses `IAmbientScopeProvider` indirectly to expose the current UoW.
- [Timing](/framework/data/timing) — `IClock` and the `AbpTimer` helpers.
- [Volo.Abp.Data](/framework/data/volo-abp-data) — `IDataFilter` and the data-filter scope are built on the same ambient-context primitives.
