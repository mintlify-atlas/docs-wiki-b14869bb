...
- **`Volo.Abp.Json.SystemTextJson`** and **`Volo.Abp.Json.Newtonsoft`** — concrete implementations. See [System.Text.Json](/framework/data/json-systemtextjson) and [Newtonsoft.Json](/framework/data/json-newtonsoft).
...
Both adapters route every parsed value through `IClock.Normalize`, so the kind of the emitted `DateTime` always matches `AbpClockOptions.Kind`. See [Timing](/framework/data/timing) for the contract.
...
If you need Newtonsoft instead — typically for HTTP compatibility with classic ABP.NET Zero clients — you replace the dependency with `AbpJsonNewtonsoftModule`. See [the Newtonsoft adapter page](/framework/data/json-newtonsoft).
...
- **For binary persistence** (cache values, blobs, event-bus payloads), use [`IObjectSerializer`](/framework/data/serialization). The two abstractions have different contracts on purpose.
...
- It does **not** define an `IObjectSerializer` adapter for free. If you need *bytes* from JSON, wrap your `IJsonSerializer` in a custom `IObjectSerializer<T>` implementation. See [Serialization](/framework/data/serialization).

## Related pages

- [System.Text.Json adapter](/framework/data/json-systemtextjson) — the default implementation and its converters.
- [Newtonsoft.Json adapter](/framework/data/json-newtonsoft) — drop-in replacement using `JsonConvert`.
- [Serialization](/framework/data/serialization) — the byte-oriented sibling abstraction.
- [Timing](/framework/data/timing) — `IClock`, the normalization source.
