...
- [Volo.Abp.Json (contract)](./json) — the shared `IJsonSerializer` API.
- [Newtonsoft adapter](./json-newtonsoft) — the alternative implementation.
- [Object Extending](./object-extending) — defines `IHasExtraProperties` and the dictionary the modifier fixes up.
- [Timing](./timing) — `IClock`, `AbpClockOptions.Kind`, `[DisableDateTimeNormalization]`.
