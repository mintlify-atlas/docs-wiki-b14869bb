...
- [JSON / IJsonSerializer](./json) — uses `IClock.Normalize` on every `DateTime` round-trip.
- [Newtonsoft adapter](./json-newtonsoft) and [System.Text.Json adapter](./json-systemtextjson) — implementation sites.
- [Threading](./threading) — `AsyncHelper`, `ICancellationTokenProvider`, and the timer utilities that consult the clock.
