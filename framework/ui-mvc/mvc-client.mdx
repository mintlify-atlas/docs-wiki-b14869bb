...
- The local event used to evict the cache comes from [`Volo.Abp.AspNetCore.Mvc`](/framework/aspnetcore/aspnetcore-mvc).
- `IDistributedCache<T>` and its serialization plumbing are documented in [`Volo.Abp.Caching`](/framework/caching/volo-abp-caching).
- The `AbpTenantClientProxy` accessed by `MvcRemoteTenantStore` is auto-generated by [`Volo.Abp.Http.Client`](/framework/httpclient/http-client), against the controller in [`mvc-ui-multitenancy`](/framework/ui-mvc/mvc-ui-multitenancy).
