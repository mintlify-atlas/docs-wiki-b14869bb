...
- The `DaprDistributedEventBus`, `IDaprSerializer`, `AbpDaprEventData` envelope and `AbpDaprEventBusOptions.PubSubName` come from [`Volo.Abp.EventBus.Dapr`](/framework/eventbus/dapr).
...
- `UseConfiguredEndpoints` and `AbpEndpointRouterOptions` belong to [`Volo.Abp.AspNetCore`](/framework/aspnetcore/aspnetcore).
