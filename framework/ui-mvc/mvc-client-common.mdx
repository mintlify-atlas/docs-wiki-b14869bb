...
- `AddStaticHttpClientProxies` and the contract assembly conventions are documented in [`Volo.Abp.Http.Client`](/framework/httpclient/http-client).
- The `ApplicationConfigurationDto`, `ApplicationLocalizationRequestDto` and the controllers behind the proxies live in [`Volo.Abp.AspNetCore.Mvc`](/framework/aspnetcore/aspnetcore-mvc).
- The `IPermissionChecker`, `ISettingProvider`, `IFeatureChecker` abstractions come from [`Volo.Abp.Authorization`](/framework/cross/authorization), [`Volo.Abp.Settings`](/framework/cross/settings) and [`Volo.Abp.Features`](/framework/cross/features) respectively.
- The localization contributor protocol is detailed in [`Volo.Abp.Localization`](/framework/cross/multi-lingual-objects).
