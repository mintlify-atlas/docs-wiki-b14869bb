...
Identical to [the AWS normalizer](/framework/blob/blob-storing-aws#awsblobnamingnormalizer):
...
- **No STS support.** If your auth model requires short-lived credentials (most enterprise S3-compatible offerings), this provider isn't the right entry point — adapt the [AWS provider](/framework/blob/blob-storing-aws) by pointing it at MinIO's S3 endpoint (the AWS SDK supports custom service URLs).

## See also

- [Volo.Abp.BlobStoring](/framework/blob/blob-storing) — the consumer pipeline.
- [Volo.Abp.BlobStoring.Aws](/framework/blob/blob-storing-aws) — same wire format, richer auth.
- [Volo.Abp.BlobStoring.Azure](/framework/blob/blob-storing-azure) — Azure equivalent.
