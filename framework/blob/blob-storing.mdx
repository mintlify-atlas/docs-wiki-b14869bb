...
The provider modules ([FileSystem](blob-storing-filesystem), [Azure](blob-storing-azure), [AWS S3](blob-storing-aws), [MinIO](blob-storing-minio), [Aliyun OSS](blob-storing-aliyun), and the application-level [Database](blob-storing-database) module) plug in by registering an `IBlobProvider` and exposing a `UseXxx(...)` extension method on `BlobContainerConfiguration`.
...
2. **Normalizes both names.** `BlobNormalizeNamingService` runs every `IBlobNamingNormalizer` registered on the configuration in order, against the **container name** and the **blob name** separately. Provider modules register their own normalizers — see [Azure](blob-storing-azure), [AWS](blob-storing-aws), [MinIO](blob-storing-minio).
...
- [Volo.Abp.BlobStoring.FileSystem](blob-storing-filesystem)
- [Volo.Abp.BlobStoring.Azure](blob-storing-azure)
- [Volo.Abp.BlobStoring.Aws](blob-storing-aws)
- [Volo.Abp.BlobStoring.Minio](blob-storing-minio)
- [Volo.Abp.BlobStoring.Aliyun](blob-storing-aliyun)
- [Volo.Abp.BlobStoring.Database (module)](blob-storing-database)
- [Volo.Abp.Imaging](imaging) for the image-processing pipeline that often pairs with BLOB containers.
