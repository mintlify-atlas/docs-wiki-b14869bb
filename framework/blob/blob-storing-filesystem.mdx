...
1. Sets `ProviderType = typeof(FileSystemBlobProvider)`. The [provider selector](/framework/blob/blob-storing#provider-selection) uses this to match.
...
Compare with [Azure](/framework/blob/blob-storing-azure#multi-tenant-and-container-name-strategy) and [AWS](/framework/blob/blob-storing-aws#multi-tenant-and-container-name-strategy), which support a separate `ContainerName` property because the storage backend has a separate namespace concept.
...
- **Multi-node deployments need a shared filesystem.** The Polly retry handles transient file-locks, not network-partition cases. For multi-instance hosts pick [Azure](/framework/blob/blob-storing-azure), [AWS](/framework/blob/blob-storing-aws), [MinIO](/framework/blob/blob-storing-minio), or [Aliyun OSS](/framework/blob/blob-storing-aliyun).

## See also

- [Volo.Abp.BlobStoring](/framework/blob/blob-storing) — the abstractions and `BlobContainer` pipeline.
- [Volo.Abp.BlobStoring.Database (module)](/framework/blob/blob-storing-database) — for persistence inside your existing RDBMS.
- [Volo.Abp.BlobStoring.Azure](/framework/blob/blob-storing-azure), [.Aws](/framework/blob/blob-storing-aws), [.Minio](/framework/blob/blob-storing-minio), [.Aliyun](/framework/blob/blob-storing-aliyun) — cloud alternatives.
