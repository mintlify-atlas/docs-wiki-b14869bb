...
- **Dapr** (`Volo.Abp.DistributedLocking.Dapr`) — talks to a Dapr lock building block. See [distributed-locking-dapr](/framework/caching/distributed-locking-dapr).
...
- **One Redis, many apps.** Different services / environments sharing the same Redis cluster won't collide if each picks a unique `KeyPrefix`. (Apply the same discipline to `AbpDistributedCacheOptions.KeyPrefix` — they are independent options classes, both prefixed for the same reason; see [caching](/framework/caching/caching).)
...
| Medallion + Redis | You already have a Redis cluster (likely if you use [caching-redis](/framework/caching/caching-redis)). Fast, ~ms acquisition, TTL-based recovery on crash. |
...
| [distributed-locking-dapr](/framework/caching/distributed-locking-dapr) (`DaprAbpDistributedLock`) | You're running on Dapr and want the lock building block to handle backend selection via component config. |
...
- [distributed-locking-dapr](/framework/caching/distributed-locking-dapr) — Dapr building-block adapter that drops in as an alternative to the Medallion adapter.
- [caching](/framework/caching/caching) — for the in-process single-flight semaphore in `DistributedCache.GetOrAddAsync`; combine with `IAbpDistributedLock` for cluster-wide single-flight.
