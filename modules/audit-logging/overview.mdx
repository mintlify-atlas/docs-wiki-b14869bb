...
`AuditLog` and `EntityChange` carry an `ExtraProperties` dictionary; both are registered for [object extensions](/framework/infrastructure/object-extensions) under module name `"AuditLogging"` (see `AuditLoggingModuleExtensionConsts`).
...
* **`GetEntityChangeListAsync`.** EF Core projects from `EntityChange` tables; the MongoDB implementation flattens nested arrays through `SelectMany`. See [MongoDB provider](./mongodb).
...
* [Audit Logging domain layer](./domain) — the four aggregates and the converter.
* [EF Core provider](./efcore) — `AbpAuditLoggingDbContext`, model builder, repository implementation, indexes.
* [MongoDB provider](./mongodb) — single collection, embedded documents, query flattening.
* [Object extensions](/framework/infrastructure/object-extensions) — how to add custom properties to `AuditLog` without forking the module.
