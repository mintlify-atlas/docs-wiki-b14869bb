...
There is no `[IgnoreMultiTenancy]` attribute on the audit-logging context. The data filter for `IMultiTenant` is therefore applied to `AuditLog` reads. In contrast, the background-jobs MongoDB context *does* declare `[IgnoreMultiTenancy]` (see [Background Jobs MongoDB](../background-jobs-module/mongodb)).
...
* [Domain layer](./domain) — the aggregates and the converter that builds the embedded tree.
* [EF Core provider](./efcore) — the relational shape with proper indexes.
* [Framework / Auditing](/framework/cross/auditing) — capture, `EntityHistorySelectors`, `IAuditingContributor`.
* [Framework / MongoDB infrastructure](/framework/data/mongodb) — `IMongoDbContextProvider`, `MongoDbRepository`.
