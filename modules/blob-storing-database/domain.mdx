...
* `ConnectionStringName` — the [Volo.Abp.Data](/framework/data/data-seeding) connection-string selector. Setting `"ConnectionStrings": { "AbpBlobStoring": "…" }` in your `appsettings.json` routes the BLOB DbContext to a different physical database. **Recommended for production**: blobs grow fast.
...
* [Framework BLOB Storing intro](/framework/blob/blob-storing) — `IBlobContainer`, `BlobProviderBase`, `BlobProviderSaveArgs`.
