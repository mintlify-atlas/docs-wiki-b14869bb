...
* `ApplyObjectExtensionMappings()` enables `IHasExtraProperties` for these entities, so your app can extend them via the [object extension system](/framework/object-extending/intro).
...
* [Volo.Abp.EntityFrameworkCore](/framework/data/ef-core) — `AbpDbContext`, `AddAbpDbContext`, model conventions.
* [Object extending](/framework/object-extending/intro) — for `ExtraProperties` mapping.
